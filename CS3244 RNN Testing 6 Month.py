...
os.chdir("C:/Users/Ong Jing Long/Desktop/CS3244 Machine Learning/Project/RNN/6mo_train_val_data_with_quarters")#need to change
...
regressorRNN.add(SimpleRNN(units=50, return_sequences=True, input_shape=(180,84), activation='tanh'))
...
training_set_scaled_x = sc.fit_transform(final_training_set.iloc[: , 1:85])
...
for indices_loop in range (0, len(train_indices)-1):#loop for every companies, note that number of element is number of companies
    X_train = []
    y_train = []
    for i in range(train_indices[indices_loop] + 180, train_indices[indices_loop + 1], 180):
        X_train.append(training_set_scaled_x[i-180:i, 0:84])
...
os.chdir("C:/Users/Ong Jing Long/Desktop/CS3244 Machine Learning/Project/RNN/6mo_test_data_with_quarters")#need to change
...
for comp in testing_companies:
    ...
    if testing_set.shape[0] < 180:
        continue
    testing_set_scaled_x = sc.transform(testing_set.iloc[: , 1:85])
    testing_set_y = testing_set.iloc[: , 0].values.reshape(-1,1)
    X_test = []
    y_test = []
    for i in range(180, len(testing_set), 180):
        X_test.append(testing_set_scaled_x[i-180:i, 0:84])
